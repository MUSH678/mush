...
local function runScript()
    ...
    local function moveTo(part, x, y, z)
        if not part then return end
        local ok = pcall(function() part.CFrame = CFrame.new(x, y, z) end)
        if not ok then pcall(function() part.Position = Vector3.new(x, y, z) end) end
    end
    local function moveOffset(part, dx, dy, dz)
        if not part then return end
        local ok = pcall(function() part.CFrame = part.CFrame * CFrame.new(dx, dy, dz) end)
        if not ok then pcall(function() part.Position = part.Position + Vector3.new(dx, dy, dz) end) end
    end
    ...
    local function findPartIn(obj)
        if hasPos(obj) then return obj end
        for _, c in ipairs(obj:GetChildren()) do
            local r = findPartIn(c)
    ...
    local tS1Btn    = makeText(uiX+PAD+5, S1_Y+BT_Y, "[ Y ]", 230, 235, 245, 10)
    ...
    local tS2Btn    = makeText(uiX+PAD+5, S2_Y+BT_Y, "[ U ]", 230, 235, 245, 10)
    local tS2Counts = makeText(uiX+PAD+2, S2_Y+BR_Y+BR_H+2, "", 130, 140, 155, 8)
    ...
    local tS3Btn    = makeText(uiX+PAD+5, S3_Y+BT_Y, "[ I ]", 230, 235, 245, 10)
    ...
    local tS4Btn    = makeText(uiX+PAD+5, S4_Y+BT_Y, "[ C ]", 230, 235, 245, 10)

    -- Anti-AFK status
    local afkSt = makeText(uiX+PAD, uiY+TOTAL_H-24, "Anti-AFK: Guarding", 100, 105, 115, 9)
    local tHotkey = makeText(uiX+PAD, uiY+TOTAL_H-14, "Y:Warp U:Roll I:Egg C:Crate R:Reset F1:Hide", 100, 105, 115, 8)
    ...
    local function updateAllPos(dx, dy)
        ...
        tS2Counts.Position = Vector2.new(uiX+PAD+2, S2_Y+BR_Y+BR_H+2)
        ...
        afkSt.Position     = Vector2.new(uiX+PAD, uiY+TOTAL_H-24)
        tHotkey.Position   = Vector2.new(uiX+PAD, uiY+TOTAL_H-14)
    end
    ...
    local function warpAndPress(hrp, pos, name)
        ...
        moveTo(hrp, pos.X, pos.Y + 3, pos.Z + 2)
        wait(0.1)
    ...
    local function togglePlantRush(hrp, folder)
        ...
        if mobName and mobPart then
            if not plantKeyPressed then
                ...
                tS1Status.Color = Color3.fromRGB(255, 200, 0)
                moveTo(hrp, mobPart.Position.X, mobPart.Position.Y + 3, mobPart.Position.Z + 2)
                wait(0.1)
    ...
    local PLOT_CACHE_TIME = 5
    local TARGET_SEEDS = {
        "Void Fruit", "Papaya", "Ghost Pepper", "Durian", "Ember Fruit", "Aurora Lotus"
    }
    local boughtCounts = {}
    for _, s in ipairs(TARGET_SEEDS) do boughtCounts[s] = 0 end
    local function getMyPlot()
        local now = tick()
        if cachedPlot and now - lastPlotCheck < PLOT_CACHE_TIME then
            return cachedPlot
        end
    ...
    local function updateCountsDisplay()
        local parts = {}
        local total = 0
        for _, s in ipairs(TARGET_SEEDS) do
            local c = boughtCounts[s] or 0
            if c > 0 then table.insert(parts, string.sub(s, 1, 4) .. ":" .. c) end
            total = total + c
        end
        if #parts > 0 then
            tS2Counts.Text = "Bought: " .. table.concat(parts, " ") .. "  |  Total: " .. total
        else
            tS2Counts.Text = ""
        end
    end
    local function runRollLoop()
        ...
        while rollEnabled do
            ...
            tS2Status.Color = Color3.fromRGB(255, 200, 0)
            moveTo(hrp2, lever.Position.X, lever.Position.Y + 3, lever.Position.Z + 2)
            wait(0.3)
            if rollEnabled then
            ...
            local pok, _ = pcall(function()
                for _, seedName in ipairs(TARGET_SEEDS) do
                    local obj = Workspace:FindFirstChild(seedName, true)
                    if obj then
                        ...
                        if pos then
                            ...
                            if hrp2 then
                                moveTo(hrp2, pos.X, pos.Y + 3, pos.Z + 2)
                                wait(0.2)
                                setrobloxinput(true); keypress(69); wait(1); keyrelease(69)
                                wait(0.1)
                            end
                            ...
                            boughtCounts[seedName] = (boughtCounts[seedName] or 0) + 1
                        end
                    else
                        allFound = false
                    end
    ...
    local function warpToEgg(pos)
        ...
        if hrp then moveTo(hrp, pos.X, pos.Y, pos.Z + 2) end
    end
    ...
    local function crateCollectAndSell(hrp, plot)
        ...
        for i = 1, math.min(2, #cratePoss) do
            ...
            if pos then
                ...
                moveTo(hrp, pos.X, pos.Y + 2, pos.Z)
                wait(0.5)
                setrobloxinput(true); keypress(69); wait(2); keyrelease(69)
                count = count + 1
                wait(0.3)
        ...
        if count > 0 then
            ...
            if sellPos then
                ...
                tS4Status.Color = Color3.fromRGB(255, 200, 0)
                moveTo(hrp, sellPos.X, sellPos.Y + 2, sellPos.Z)
                wait(0.5)
                setrobloxinput(true); keypress(69); wait(2); keyrelease(69)
    ...
    while true do
        ...
        if justPressed then
            ...
            -- S3 egg type pills (mouse click still works for these)
        ...
        -- R = reset bought counts
        if isKeyJustPressed(82) then
            for _, s in ipairs(TARGET_SEEDS) do boughtCounts[s] = 0 end
            tS2Counts.Text = "Reset!"
            tS2Status.Text = "Counts reset"
            tS2Status.Color = Color3.fromRGB(255, 200, 0)
        end
        ...
        if not didAction and eggEnabled then
            ...
            if eggHasActive and now - lastEggTime >= EGG_COOLDOWN then
                local char = Players.LocalPlayer and Players.LocalPlayer.Character
                local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local eggs = findEggs()
