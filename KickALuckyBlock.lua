...
local function isBasePart(obj)
    ...
    return cn == "Part" or cn == "MeshPart" or cn == "UnionOperation" or cn == "WedgePart" or cn == "CylinderPart" or cn == "BallPart" or cn == "TrussPart" or cn == "CornerWedgePart"
end

local function isGuiObject(obj)
    if not obj then return false end
    local ok = pcall(function() return obj.AbsolutePosition end)
    return ok
end
...
local function safeGetAttribute(obj, name)
    ...
    local ok, val = pcall(function() return obj:GetAttribute(name) end)
...
local AccentThemes = {
    ["Green"] = { accent = Color3.fromRGB(54, 248, 87), accentSoft = Color3.fromRGB(40, 180, 70) },
    ["Blue"] = { accent = Color3.fromRGB(70, 170, 255), accentSoft = Color3.fromRGB(50, 120, 195) },
    ["Red"] = { accent = Color3.fromRGB(255, 92, 92), accentSoft = Color3.fromRGB(190, 68, 68) },
    ["Orange"] = { accent = Color3.fromRGB(255, 164, 58), accentSoft = Color3.fromRGB(195, 116, 38) },
    ["Pink"] = { accent = Color3.fromRGB(255, 105, 180), accentSoft = Color3.fromRGB(196, 73, 136) },
    ["White"] = { accent = Color3.fromRGB(235, 235, 235), accentSoft = Color3.fromRGB(155, 155, 155) },
    ["Black"] = { accent = Color3.fromRGB(18, 18, 18), accentSoft = Color3.fromRGB(48, 48, 48) },
    ["Rainbow"] = { accent = Color3.fromRGB(54, 248, 87), accentSoft = Color3.fromRGB(40, 180, 70) },
}
...
local function NewBucket()
    return { objects = {}, visible = false, built = false, deferVisible = false }
end
...
local UI = {
    ...
    needsRedraw = true,
    shellDirty = true,
    tabDirty = {true, true, true, true, true},
    hitboxes = {},
    hoveredId = nil,
    ...
    settingsTabIndex = 5,
    activeTweenId = 0,
    pendingTweenTargets = nil,
    tweenWorkerRunning = false,
    autoCollectReturning = false,
    autoCollectOutOfBoundsSince = nil,
    autoKickSequenceActive = false,
    autoKickMinigameDown = false,
    autoKickNextAttemptAt = 0,
    pop2xNextAttemptAt = 0,
    configFolderName = "KickALuckyBlock",
    ...
    window = { x = 220, y = 140, width = 620, height = 352, headerHeight = 42, footerHeight = 20, padding = 16, sidebarWidth = 185 },
    scroll = { offset = 0, max = 0 },
    shellBucket = NewBucket(),
    loadingBucket = NewBucket(),
    tabBuckets = { NewBucket(), NewBucket(), NewBucket(), NewBucket(), NewBucket() },
    tabs = { {name = "HOME"}, {name = "AUTO FARM"}, {name = "TELEPORTS"}, {name = "MISC"}, {name = "SETTINGS"} },
    controls = {
        {
            {id = "uptime_stat", type = "stat", label = "Uptime", value = "0s"},
            {id = "profile_stat", type = "stat", label = "Profile", value = LocalPlayer and LocalPlayer.Name or "Unknown"},
            {id = "game_stat", type = "stat", label = "Game", value = "Kick A Lucky Block"},
            {id = "last_save_stat", type = "stat", label = "Last Config Save", value = "Never"},
        },
        {
            {id = "auto_kick", type = "toggle", label = "Auto Kick", description = "", value = false},
            {id = "auto_collect", type = "toggle", label = "Auto Collect", description = "", value = false},
        },
        {
            {id = "misc_teleports", type = "dropdown_single", label = "Misc Teleports", description = "", value = "Select", options = {"Select", "Base", "Run Shop", "Weights Shop"}},
        },
        {
            {id = "pop_2x", type = "toggle", label = "Pop 2x", description = "", value = false},
        },
        {
            ...
            {id = "copy_placeid", type = "button", label = "Copy PlaceId", description = "", accent = "accent"},
        },
    },
}
...
function UI:SetDrawingProperty(obj, property, value, epsilon)
    ...
    if type(current) == "number" and type(value) == "number" then
        if NearlyEqual(current, value, epsilon) then return false end
...
function UI:CreateImage(bucket, data, x, y, w, h, transparency)
    if self.logoImageUnsupported then return nil end
...
function UI:AnimateValue(store, key, target, speed, dt, defaultValue)
    ...
    if current == nil then current = defaultValue ~= nil and defaultValue or target end
...
function UI:TransformPoint(x, y)
    ...
    if math.abs(scale - 1) < 0.001 then return x, y end
...
function UI:TransformSize(w, h)
    local scale = self.renderScale or 1
    if math.abs(scale - 1) < 0.001 then return w, h end
...
function UI:UpdateAnimations(dt)
    ...
    if math.abs(self.uiAlpha - alphaTarget) < 0.002 then self.uiAlpha = alphaTarget else shellAnimating = true; tabAnimating = true end
    ...
    if math.abs(self.uiOffset - offsetTarget) >= 0.02 then shellAnimating = true; tabAnimating = true else self.uiOffset = offsetTarget end
    ...
    if math.abs((self.renderScale or scaleTarget) - scaleTarget) >= 0.002 then shellAnimating = true; tabAnimating = true else self.renderScale = scaleTarget end
    ...
    if math.abs((self.activeTabHighlightY or activeTabTargetY) - activeTabTargetY) >= 0.02 then shellAnimating = true else self.activeTabHighlightY = activeTabTargetY end
    ...
    if math.abs((self.animatedScrollThumbY or thumbTargetY) - thumbTargetY) >= 0.02 then tabAnimating = true else self.animatedScrollThumbY = thumbTargetY end

    if shellAnimating then self:MarkShellDirty() end
    if tabAnimating then self:MarkCurrentTabDirty() end
end
...
function UI:RegisterHitbox(id, x, y, w, h, callback, priority)
    table.insert(self.hitboxes, { id = id, x = x, y = y, w = w, h = h, callback = callback, priority = priority or 0 })
end
...
function UI:IsMousePressed()
    local ok, pressed = pcall(function() return UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) end)
    if ok then return pressed == true end
    if type(ismouse1pressed) == "function" then
        local pressOk, result = pcall(ismouse1pressed)
        if pressOk then return result == true end
    end
    return false
end
...
function UI:RefreshHomeStats(force)
    local now = tick()
    if not force and self.selectedTab ~= 1 then return end
    if not force and (now - (self.lastHomeStatRefresh or 0)) < 1 then return end
    self.lastHomeStatRefresh = now
    self:SetStatValue("uptime_stat", self:FormatDuration(now - (self.launchTick or now)))
    self:SetStatValue("profile_stat", LocalPlayer and LocalPlayer.Name or "Unknown")
    self:SetStatValue("game_stat", tostring(game and game.PlaceId or 0))
...
function UI:UpdateRainbowAccent()
    ...
    if not accentControl or accentControl.value ~= "Rainbow" then return end
    ...
    local accentChanged = not previousAccent or math.abs(previousAccent.R - accent.R) > 0.01 or math.abs(previousAccent.G - accent.G) > 0.01 or math.abs(previousAccent.B - accent.B) > 0.01
    local accentSoftChanged = not previousAccentSoft or math.abs(previousAccentSoft.R - accentSoft.R) > 0.01 or math.abs(previousAccentSoft.G - accentSoft.G) > 0.01 or math.abs(previousAccentSoft.B - accentSoft.B) > 0.01
    if accentChanged or accentSoftChanged then
        self:MarkShellDirty()
        self:MarkCurrentTabDirty()
...
function UI:GetConfigFolderPath() return self.configFolderName or "KickALuckyBlock" end
function UI:GetConfigFilePath() return self:GetConfigFolderPath() .. "/" .. (self.configFileName or "config.json") end
...
function UI:ApplyConfigData(data)
    if type(data) ~= "table" or type(data.controls) ~= "table" then return end
        ...
        if type(saved) == "table" then
            ...
            if control then
                if control.type == "dropdown_multi" and type(saved.selected) == "table" then
                    local applied = {}
                    for option, isSelected in pairs(saved.selected) do applied[tostring(option)] = isSelected == true end
    ...
    local widthControl = self:GetControl(self.settingsTabIndex or 5, "ui_scale")
    if widthControl and widthControl.value then self.window.width = widthControl.value end
    local heightControl = self:GetControl(self.settingsTabIndex or 5, "ui_height")
    if heightControl and heightControl.value then self.window.height = heightControl.value end
    local accentControl = self:GetControl(self.settingsTabIndex or 5, "accent_theme")
    if accentControl then self:ApplyAccentTheme(accentControl.value) end
    self:MarkShellDirty()
    for tabIndex = 1, #self.tabDirty do self:MarkTabDirty(tabIndex) end
end

function UI:SaveConfig(force)
    if not force and not self:IsAutoSaveConfigEnabled() then return false end
    if type(writefile) ~= "function" or not HttpService then return false end
    self:EnsureConfigStorage()
    local encodeOk, payload = pcall(function() return HttpService:JSONEncode(self:BuildConfigData()) end)
    if not encodeOk or not payload then return false end
    local writeOk = pcall(writefile, self:GetConfigFilePath(), payload)
    if writeOk then self.lastConfigSaveTick = tick(); self:SetStatValue("last_save_stat", "0s ago") end
    return writeOk
end

function UI:LoadConfig()
    if type(readfile) ~= "function" or type(isfile) ~= "function" or not HttpService then return false end
    self:EnsureConfigStorage()
    ...
    if not readOk or not payload or payload == "" then return false end
    local decodeOk, data = pcall(function() return HttpService:JSONDecode(payload) end)
...
function UI:NotifyPlayer(message, title, duration)
    if type(notify) == "function" then pcall(notify, tostring(message or ""), tostring(title or "MUSH"), tonumber(duration) or 3) end
end

function UI:GetTweenSpeed()
    local control = self:GetControl(self.settingsTabIndex or 5, "tween_speed")
...
function UI:GetReadableOwnerName(plot)
    if not plot then return nil end
    local owner = safeGetAttribute(plot, "Owner")
    if not owner then return nil end
    local ownerText = tostring(owner)
    if ownerText == "" or ownerText:match("^[%w_]+$") == nil then return nil end
    return ownerText
end

function UI:GetMyBaseSpawnPart()
    local plots = workspace:FindFirstChild("Plots")
    if not plots then return nil end
    for _, plot in ipairs(plots:GetChildren()) do
        local ownerName = self:GetReadableOwnerName(plot)
        if ownerName == LocalPlayer.Name then
            local spawnPart = plot:FindFirstChild("SpawnPart")
            if spawnPart and isBasePart(spawnPart) then return spawnPart end
        end
    end
    return nil
end

local AUTO_COLLECT_WALK_TARGET = Vector3.new(713, 3, 232)
local AUTO_KICK_POSITION = Vector3.new(706, 3, 232)
local AUTO_KICK_MINIGAME_CLICK_HEIGHT = 435
...
local function WriteVector3(address, offset, vector3)
    if not address or type(memory_write) ~= "function" then return false end
...
function WalkToService.WalkTo(humanoid, targetPosition)
    if not humanoid or not humanoid.Address or typeof(targetPosition) ~= "Vector3" then return false end
    ...
    local function writeWalkTarget()
        local currentRoot = WalkToService.character and WalkToService.character:FindFirstChild("HumanoidRootPart")
        if not currentRoot then return false end
        local delta = WalkToService.currentTarget - currentRoot.Position
        local distance = delta.Magnitude
        if distance <= 4 then WalkToService.Stop(); return false end
        local direction = delta.Unit
        local humAddress = WalkToService.humanoid and WalkToService.humanoid.Address
        if not humAddress then WalkToService.Stop(); return false end
    ...
    if not writeWalkTarget() then return false end
    WalkToService.updateThread = task.spawn(function()
        while WalkToService.isWalking do
            if not writeWalkTarget() then break end
            task.wait(0.1)
        end
    end)
    return true
...
function UI:WalkToPosition(targetPosition)
...
function UI:StopWalking() WalkToService.Stop() end
function UI:IsWalking() return WalkToService.isWalking == true end

function UI:GetKickButton()
    local playerGui = LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui")
    local hud = playerGui and playerGui:FindFirstChild("HUD")
    local button = hud and hud:FindFirstChild("KickButton")
    if button and isGuiObject(button) then return button end
    return nil
end

function UI:GetPop2xButton()
    local playerGui = LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui")
    local kickUpgrades = playerGui and playerGui:FindFirstChild("KickUpgrades")
    if not kickUpgrades then return nil end
    local matches = {}
    for _, descendant in ipairs(kickUpgrades:GetDescendants()) do
        if descendant.Name == "Bonus" and isGuiObject(descendant) then
            matches[#matches + 1] = descendant
        end
    end
    if #matches < 2 then return nil end
    for _, bonus in ipairs(matches) do
        if bonus.Visible == true and bonus.AbsoluteSize.X > 0 and bonus.AbsoluteSize.Y > 0 then
            return bonus
        end
    end
    if matches[2] then return matches[2] end
    return matches[#matches]
end

function UI:GetKickMovingBar()
    local playerGui = LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui")
    local minigame = playerGui and playerGui:FindFirstChild("KickMinigame")
    local bar = minigame and minigame:FindFirstChild("Bar")
    local movingBar = bar and bar:FindFirstChild("MovingBar")
    if movingBar and isGuiObject(movingBar) then return movingBar end
    return nil
end

function UI:IsKickMinigameActive()
    local movingBar = self:GetKickMovingBar()
    if not movingBar then return false end
    return movingBar.AbsoluteSize.Y > 1
end

function UI:MoveMouseToGuiObject(guiObject, debugPrefix)
    if not guiObject or type(mousemoveabs) ~= "function" then return false end
    local center = guiObject.AbsolutePosition + (guiObject.AbsoluteSize / 2)
    return pcall(mousemoveabs, center.X, center.Y)
end

function UI:PerformKickButtonClick()
    if self.autoCollectReturning and self:IsWalking() then return false end
    if type(mouse1click) ~= "function" then return false end
    local button = self:GetKickButton()
    if not button then return false end
    local moved = self:MoveMouseToGuiObject(button)
    if not moved then return false end
    task.wait(0.05)
    if type(mousemoverel) == "function" then pcall(mousemoverel, 5, 0) end
    local ok = pcall(mouse1click)
    return ok
end

function UI:StartAutoKickSequence()
    if self.autoKickSequenceActive then return end
    self.autoKickSequenceActive = true
    task.spawn(function()
        local success = false
        local arrived = false
        local rootPart = self:GetLocalRootPart()
        local distanceToTarget = rootPart and (rootPart.Position - AUTO_KICK_POSITION).Magnitude or math.huge
        if distanceToTarget > 3 then
            pcall(function() self:TweenToPosition(AUTO_KICK_POSITION) end)
        else
            arrived = true
        end
        local startedAt = tick()
        while not arrived and self.isRunning and (tick() - startedAt) < 8 do
            local currentRoot = self:GetLocalRootPart()
            if currentRoot and (currentRoot.Position - AUTO_KICK_POSITION).Magnitude <= 3 then
                arrived = true
                break
            end
            task.wait(0.1)
        end
        if self.isRunning and arrived then
            local ok, result = pcall(function() return self:PerformKickButtonClick() end)
            success = ok and result == true
        end
        self.autoKickNextAttemptAt = tick() + (success and 0.75 or 1.5)
        self.autoKickSequenceActive = false
    end)
end

function UI:HandleAutoKickMinigame()
    local control = self:GetControl(2, "auto_kick")
    if not control or control.value ~= true then self.autoKickMinigameDown = false; return end
    if self.autoCollectReturning and self:IsWalking() then self.autoKickMinigameDown = false; return end
    local movingBar = self:GetKickMovingBar()
    if movingBar then
        local height = movingBar.AbsoluteSize.Y
        if height >= AUTO_KICK_MINIGAME_CLICK_HEIGHT then
            if not self.autoKickMinigameDown and type(mouse1click) == "function" then
                self.autoKickMinigameDown = true
                pcall(mouse1click)
            end
            return
        end
        self.autoKickMinigameDown = false
    end
    self.autoKickMinigameDown = false
end

function UI:HandleAutoKickSequence()
    local control = self:GetControl(2, "auto_kick")
    if not control or control.value ~= true then
        self.autoKickSequenceActive = false
        return
    end
    if self.autoCollectReturning and self:IsWalking() then return end
    local movingBar = self:GetKickMovingBar()
    if movingBar and movingBar.AbsoluteSize.Y > 1 then return end
    if self.autoKickSequenceActive or tick() < (self.autoKickNextAttemptAt or 0) then return end
    self:StartAutoKickSequence()
end

function UI:EnsureTweenWorker()
    if self.tweenWorkerRunning then return end
    self.tweenWorkerRunning = true
    task.spawn(function()
        local stepWait = 1 / 120
        while self.isRunning do
            local tweenId = self.activeTweenId or 0
            local targets = self.pendingTweenTargets
            if type(targets) ~= "table" or #targets <= 0 then
                task.wait(0.02)
            else
                local speed = self:GetTweenSpeed()
                for _, targetPosition in ipairs(targets) do
                    if not self.isRunning or self.activeTweenId ~= tweenId then break end
                    if typeof(targetPosition) == "Vector3" then
                        local lastTick = tick()
                        while self.isRunning and self.activeTweenId == tweenId do
                            local currentRoot = self:GetLocalRootPart()
                            if not currentRoot then break end
                            local now = tick()
                            local dt = math.max(stepWait, now - lastTick)
                            lastTick = now
                            local currentPosition = currentRoot.Position
                            local offset = targetPosition - currentPosition
                            local distance = offset.Magnitude
                            if distance <= 2 then
                                currentRoot.Position = targetPosition
                                break
                            end
                            local step = math.min(distance, speed * dt)
                            currentRoot.Position = currentPosition + offset.Unit * step
                            task.wait(stepWait)
                        end
                    end
                end
                if self.activeTweenId == tweenId then self.pendingTweenTargets = nil end
            end
        end
        self.tweenWorkerRunning = false
    end)
end

function UI:TweenToPositions(targetPositions)
    local rootPart = self:GetLocalRootPart()
    if not rootPart or type(targetPositions) ~= "table" or #targetPositions <= 0 then return false end
    local queuedTargets = {}
    for _, targetPosition in ipairs(targetPositions) do
        if typeof(targetPosition) == "Vector3" then queuedTargets[#queuedTargets + 1] = targetPosition end
    end
    if #queuedTargets <= 0 then return false end
    self.activeTweenId = (self.activeTweenId or 0) + 1
    self.pendingTweenTargets = queuedTargets
    self:EnsureTweenWorker()
    return true
end

function UI:TweenToPosition(targetPosition) return self:TweenToPositions({targetPosition}) end

function UI:HandleMiscTeleport(option)
    local destination = nil
    if option == "Base" then
        local spawnPart = self:GetMyBaseSpawnPart()
        if not spawnPart then self:NotifyPlayer("Could not find your base", "mush", 3); return end
        destination = spawnPart.Position + Vector3.new(0, 3, 0)
    elseif option == "Run Shop" then
        destination = Vector3.new(727, 3, 269)
    elseif option == "Weights Shop" then
        destination = Vector3.new(727, 3, 193)
    end
    if not destination then return end
    if self:TweenToPosition(destination) then self:NotifyPlayer("Teleporting to " .. option, "mush", 2) end
end

function UI:HandleAutoCollect()
    local control = self:GetControl(2, "auto_collect")
    if not control or control.value ~= true then
        self.autoCollectReturning = false
        self.autoCollectOutOfBoundsSince = nil
        self:StopWalking()
        return
    end
    local rootPart = self:GetLocalRootPart()
    if not rootPart then return end
    if rootPart.Position.X >= 690.5 then
        self.autoCollectReturning = false
        self.autoCollectOutOfBoundsSince = nil
        self:StopWalking()
        return
    end
    if self.autoCollectReturning then return end
    local now = tick()
    if not self.autoCollectOutOfBoundsSince then
        self.autoCollectOutOfBoundsSince = now
        self.autoCollectReturning = true
        self:WalkToPosition(AUTO_COLLECT_WALK_TARGET)
        return
    end
    if (now - self.autoCollectOutOfBoundsSince) < 6 then return end
    self.autoCollectReturning = true
    self.autoCollectOutOfBoundsSince = nil
    self:WalkToPosition(AUTO_COLLECT_WALK_TARGET)
end

function UI:HandlePop2x()
    local control = self:GetControl(4, "pop_2x")
    if not control or control.value ~= true then return end
    if tick() < (self.pop2xNextAttemptAt or 0) then return end
    local bonusButton = self:GetPop2xButton()
    if not bonusButton then return end
    if type(mouse1click) ~= "function" then return end
    local moved = self:MoveMouseToGuiObject(bonusButton)
    if not moved then return end
    task.wait(0.05)
    if type(mousemoverel) == "function" then pcall(mousemoverel, 5, 0) end
    pcall(mouse1click)
    self.pop2xNextAttemptAt = tick() + 0.5
end
...
function UI:MarkShellDirty()
    if not self.shellDirty then self.shellDirty = true end
    if not self.needsRedraw then self.needsRedraw = true end
end

function UI:MarkTabDirty(tabIndex)
    if not self.tabDirty[tabIndex] then self.tabDirty[tabIndex] = true end
    if not self.needsRedraw then self.needsRedraw = true end
end

function UI:MarkCurrentTabDirty() self:MarkTabDirty(self.selectedTab) end

function UI:SetScrollOffset(offset)
    local nextOffset = math.clamp(math.floor(offset), 0, self.scroll.max)
    if self.scroll.offset == nextOffset then return false end
...
function UI:ToggleDropdown(controlId)
    ...
    if self.expandedDropdown == controlId then self.expandedDropdown = nil else self.expandedDropdown = controlId end
    if previousExpanded ~= self.expandedDropdown then self:UpdateScrollLimit(); self:MarkCurrentTabDirty() end
end
...
function UI:SetSliderValue(control, percent)
    ...
    if control.value ~= steppedValue then
        ...
        if control.id == "ui_scale" then self.window.width = steppedValue; self:MarkShellDirty()
        elseif control.id == "ui_height" then self.window.height = steppedValue; self:MarkShellDirty()
        elseif control.id == "tween_speed" and steppedValue > 120 and not self.tweenSpeedWarned then
            self.tweenSpeedWarned = true
            self:NotifyPlayer("Tween speeds above 120 may get you banned", "mush", 4)
        elseif control.id == "tween_speed" then self.tweenSpeedWarned = false end
        self:MarkCurrentTabDirty()
...
function UI:GetFormattedDropdownValue(control)
    if control.type == "dropdown_single" then return tostring(control.value or "Select") end
    local active = {}
    for _, option in ipairs(control.options or {}) do
        if control.selected and control.selected[option] then table.insert(active, option) end
    end
    if #active == 0 then return "None" end
    if #active <= 2 then return table.concat(active, ", ") end
...
function UI:HandleControlClick(control)
    if control.type == "toggle" or control.type == "checkbox" then
        ...
        if control.id == "ui_animations" then self.needsRedraw = true
        elseif control.id == "auto_kick" and control.value == true then
            self:NotifyPlayer("Press Z + X together to disable it instantly.", "mush", 8)
        end
        self:SaveConfig(control.id == "auto_save_config")
    elseif control.type == "button" then
        control.flashUntil = tick() + 0.35
        if control.id == "copy_placeid" then
            if type(setclipboard) == "function" then pcall(setclipboard, tostring(game and game.PlaceId or 0)) end
            self:NotifyPlayer("PlaceId copied to clipboard", "mush", 3)
        else
            self:NotifyPlayer(control.label .. " clicked", "mush", 2)
        end
...
function UI:HandleDropdownOption(control, option)
    ...
    if control.id == "accent_theme" then self:ApplyAccentTheme(control.value); self:MarkShellDirty()
    elseif control.id == "misc_teleports" then self:HandleMiscTeleport(control.value); control.value = "Select" end
...
function UI:HandleHotkeys()
    ...
    local now = tick()
    if now - self.lastToggleTick < 0.2 then return end
    local okZ, zPressed = pcall(iskeypressed, VK_Z)
    local okX, xPressed = pcall(iskeypressed, VK_X)
    if okZ and okX and zPressed and xPressed then
        local autoKickControl = self:GetControl(2, "auto_kick")
        if autoKickControl and autoKickControl.value == true then
            autoKickControl.value = false
            self.autoKickMinigameDown = false
            self.autoKickSequenceActive = false
            self.autoKickNextAttemptAt = 0
            self:NotifyPlayer("Auto Kick disabled", "mush", 3)
            self:MarkCurrentTabDirty()
    ...
    if okZ and zPressed then
        self.uiVisible = not self.uiVisible
        if self.uiVisible then self.contentAlpha = 0.85; self.contentOffset = 4 end
        self:MarkShellDirty()
        self:MarkCurrentTabDirty()
        self.lastToggleTick = now
    end
...
function UI:HandleTransientEffects()
    ...
    if dirty then self:MarkCurrentTabDirty() end
end
...
function UI:HandleInput()
    ...
    elseif self.draggingWindow and not mouseDown then self.draggingWindow = false end
    if self.draggingSlider and self.activeSlider and mouseDown then
        self:SetSliderFromMouse(self.activeSlider, mousePos.X)
    elseif self.draggingSlider and not mouseDown then self.draggingSlider = false; self.activeSlider = nil end
    ...
    elseif self.draggingScroll and not mouseDown then self.draggingScroll = false end
    local hovered = nil
    ...
    if mouseDown and not self.lastMousePressed and not clickedHitbox then
        local canDragWindow = (not self.isBooting and self:IsPointInMainWindow(mousePos)) or (self.isBooting and self:IsPointInLoadingWindow(mousePos))
        if canDragWindow then self.draggingWindow = true; self.dragOffsetX = mousePos.X - self.window.x; self.dragOffsetY = mousePos.Y - self.window.y end
    end
    if hovered ~= self.hoveredId then
        ...
        if self.isBooting then self.needsRedraw = true else
            if previousIsShell or hoveredIsShell then self:MarkShellDirty() end
            if (previousHovered ~= nil and not previousIsShell) or (hovered ~= nil and not hoveredIsShell) then self:MarkCurrentTabDirty() end
        end
    end
...
function UI:BuildShell()
    ...
    local iconX = wx + 16
    local iconY = wy + 16
    self:CreateText(bucket, "mush", iconX + 18, iconY + 6, Theme.text, 17, false)
    self:CreateText(bucket, "Kick A Lucky Block", iconX + 18, iconY + 22, Theme.textDim, 14, false)
    ...
    for tabIndex, tab in ipairs(self.tabs) do
        ...
        self:CreateText(bucket, tab.name, wx + 24, buttonY + 7, LerpColor(LerpColor(Theme.textDim, Theme.text, hoverAmount), Theme.success, selectedAmount), 13, false)
        self:RegisterHitbox("tab_" .. tostring(tabIndex), buttonX, buttonY, buttonW, buttonH, function()
            if self.selectedTab ~= tabIndex then
                self.selectedTab = tabIndex
                self.expandedDropdown = nil
...
function UI:BuildLoadingScreen()
    ...
    self:CreateText(bucket, self.bootReady and "Ready" or "Loading...", loadX + math.floor(loadW * 0.5), loadY + 92, self.bootReady and Theme.text or Theme.textDim, 13, true)
...
function UI:BuildActiveTab()
    ...
    for _, control in ipairs(self:GetActiveControls()) do
        ...
        if shouldDrawCard then
            if control.type == "stat" then
                ...
                if control.type == "toggle" then self:DrawToggle(bucket, control, cardX + cardW - 32, drawY + 14, 16, 16)
                elseif control.type == "checkbox" then self:DrawCheckbox(bucket, control, cardX + cardW - 32, drawY + 14, 16, 16)
                elseif control.type == "button" then self:DrawButton(bucket, control, cardX + cardW - 128, drawY + 10, 114, 24)
                elseif control.type == "slider" then self:DrawSlider(bucket, control, cardX + 16, drawY + 32, cardW - 86, 18)
                else self:DrawDropdown(bucket, control, cardX + 176, drawY + 10, cardW - 192, 24) end
                if control.type == "slider" then
                    self:RegisterHitbox(control.id, cardX + 16, drawY + 26, cardW - 62, 24, function()
                        self.draggingSlider = true; self.activeSlider = control; self:SetSliderFromMouse(control, self:GetMouseLocation().X)
                    end, 2)
                elseif control.type ~= "stat" then
                    self:RegisterHitbox(control.id, cardX, drawY, cardW, baseCardHeight, function() self:HandleControlClick(control) end, 1)
                end
                if expanded then
                    local listY = drawY + baseCardHeight
                    local listH = math.max(0, extraHeight - 2)
                    ...
                    for optionIndex, option in ipairs(control.options) do
                        local rowY = listY + 4 + ((optionIndex - 1) * 30)
                        local selected = control.type == "dropdown_single" and control.value == option or (control.selected and control.selected[option] == true)
                        local optionHoverId = control.id .. "_opt_" .. optionIndex
                        ...
                        if rowY + 26 <= listY + listH + 2 then
                            ...
                            if selected then self:CreateSquare(bucket, cardX + cardW - 24, rowY + 8, 8, 8, Theme.accent, dropdownProgress, true) end
                            if dropdownProgress > 0.96 then
                                self:RegisterHitbox(optionHoverId, cardX + 4, rowY, cardW - 8, 26, function() self:HandleDropdownOption(control, option) end, 2)
                            end
                        end
                    end
                end
    ...
    if self.scroll.max > 0 and self.scroll.offset < (self.scroll.max - 2) then
        self:CreateText(bucket, "v", x + math.floor((width - 16) * 0.5), y + height - 14, Theme.textDim, 12, true)
    end
    if self.scroll.max <= 0 then
        ...
        self:RegisterHitbox("scrollbar_thumb", scrollbarX - 10, animatedThumbY - 2, scrollbarW + 20, thumbH + 4, function()
            self.draggingScroll = true; self.scrollDragOffset = self:GetMouseLocation().Y - animatedThumbY
        end, 3)
    end
    bucket.built = true
    bucket.deferVisible = false
...
function UI:Render()
    ...
    if self.isBooting then
        ...
        for _, bucket in ipairs(self.tabBuckets) do self:HideBucket(bucket) end
    ...
    for tabIndex, bucket in ipairs(self.tabBuckets) do
        if tabIndex ~= self.selectedTab then self:HideBucket(bucket) end
    end
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if UI.isBooting then
        ...
        if math.abs((UI.bootPanelScale or 1) - 1) < 0.001 then UI.bootPanelScale = 1 end
        ...
        UI.bootVisualProgress = math.max(minProgress, LerpNumber(UI.bootVisualProgress or 0, UI.bootVisualTarget or minProgress, SmoothFactor(2.2 + (math.random() * 0.8), dt or (1 / 60))))
        if timeProgress >= 1 then UI.bootVisualProgress = 1 end
        if (tick() - UI.bootStartedAt) >= UI.bootDuration and (UI.bootVisualProgress or 0) >= 0.999 then
            UI.bootReady = true
        end
        if math.abs((UI.bootVisualProgress or 0) - oldProgress) >= 0.002 or math.abs((UI.bootVisualTarget or 0) - oldTarget) >= 0.01 or math.abs((UI.bootPanelScale or 0.4) - oldPanelScale) >= 0.002 or (UI.bootReady == true) ~= oldReady then
            UI.needsRedraw = true
        end
    end
    pcall(function() UI:UpdateAnimations(dt or (1 / 60)) end)
    pcall(function() UI:HandleHotkeys() end)
    pcall(function() UI:RefreshHomeStats(false) end)
    if UI.isBooting then
        pcall(function() UI:HandleInput() end)
    elseif UI.uiVisible and (UI.uiAlpha or 0) > 0.9 and (UI.renderScale or 1) > 0.98 then
        pcall(function() UI:HandleInput() end)
        pcall(function() UI:HandleTransientEffects() end)
    end
    if UI.needsRedraw then pcall(function() UI:Render() end) end
end)

task.spawn(function()
    while UI.isRunning do
        pcall(function() UI:HandleAutoCollect() end)
        task.wait(0.05)
    end
end)
...
task.spawn(function()
    while UI.isRunning do
        pcall(function() UI:HandleAutoKickMinigame() end)
        task.wait(0.01)
    end
end)
...
task.spawn(function()
    while UI.isRunning do
        pcall(function() UI:HandleAutoKickSequence() end)
        task.wait(0.1)
    end
end)
...
task.spawn(function()
    while UI.isRunning do
        pcall(function() UI:HandlePop2x() end)
        task.wait(0.1)
    end
end)
